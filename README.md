...
Each selected script's own leading comment header (project1998-testclient's convention) can override two
of the defaults above: `# budget: <N>s` sets that script's timeout, taking precedence over
`-ScriptTimeoutSec` (default or explicit) for that script alone (a header over 3600s is a refusal before
