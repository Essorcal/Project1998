...
/// <c>re/extract_lua_spawns.py</c> into <c>game-data/AreaSpawns.csv</c>.</summary>
...
public static partial class Content
{
    ...
    {
        Maps = LoadMaps(ResolvePath("NEXUS_MAP_INDEX", "map_index.csv"));
        MobFleeOverrides = LoadMobFlees(ResolvePath("NEXUS_MOB_FLEES", "MobFlees.csv"));   // BEFORE Mobs: LoadMobs folds it in
        Mobs = LoadMobs(ResolvePath("NEXUS_MOBS", "mobs.csv"));
        Items = LoadItems(ResolvePath("NEXUS_ITEMS", "Items.csv"));
        Warps = LoadWarps(ResolvePath("NEXUS_WARPS", "Warps.csv"));   // needs Maps
        Spawns = LoadSpawns(ResolvePath("NEXUS_SPAWNS", "Spawns.csv"));
        ...
        AreaSpawns = LoadAreaSpawns(ResolvePath("NEXUS_AREASPAWNS", "AreaSpawns.csv"))
            .Concat(LoadAreaSpawns(ResolvePath("NEXUS_AREASPAWNS_TRAP", "AreaSpawnsTrap.csv")))
            .ToList();
        var npcs = LoadNpcs(ResolvePath("NEXUS_NPCS", "NPCs.csv"));   // needs Maps
        ...
        MinorQuests = LoadMinorQuests(ResolvePath("NEXUS_MINORQUESTS", "MinorQuests.csv"));
        ShopStock = LoadShopStock(ResolvePath("NEXUS_SHOPSTOCK", "ShopStock.csv"));
        Paths = LoadPaths(ResolvePath("NEXUS_PATHS", "Paths.csv"));
        LevelExp = LoadLevelExp(ResolvePath("NEXUS_LEVELEXP", "LevelExp.csv"));
        SpellLevelOverrides = LoadSpellLevels(ResolvePath("NEXUS_SPELL_LEVELS", "SpellLevels.csv"));   // BEFORE Spells: LoadSpells reads it
        Spells = LoadSpells(ResolvePath("NEXUS_SPELLS", "Spells.csv"));
        ...
        SpellFx = LoadSpellFx(ResolvePath("NEXUS_SPELL_FX", "spell_effects.csv"));
        SpellTexts = LoadSpellTexts(ResolvePath("NEXUS_SPELL_TEXT", "SpellText.csv"));
        SpellCosts = LoadSpellCosts(ResolvePath("NEXUS_SPELL_COSTS", "SpellLearnCosts.csv"));
        LookPalettes = LoadLookPalettes(ResolvePath("NEXUS_MOB_PALETTES", "MobLookPalettes.csv"));
        ArmorDyeRamps = LoadArmorDyeRamps(ResolvePath("NEXUS_ARMOR_DYE_RAMPS", "ArmorDyeRamps.csv"));
        MapMeta = LoadMapMeta(ResolvePath("NEXUS_MAPS_FULL", "Maps.csv"));   // region + warpOut for Gateway
        MobDrops = LoadMobDrops(ResolvePath("NEXUS_MOB_DROPS", "MobDrops.csv"));
        CraftingToggleOverrides = LoadCraftingToggles(ResolvePath("NEXUS_CRAFTING_TOGGLES", "CraftingToggles.csv"));
        Shared.EraCalendar.Reload();   // era date + windows live in Shared (login server shares them)
        WarpQuestLocks = LoadWarpQuestLocks(ResolvePath("NEXUS_WARP_QUEST_LOCKS", "WarpQuestLocks.csv"));
        var mythicCaves = LoadMythicCaves(ResolvePath("NEXUS_MYTHIC_CAVES", "MythicCaves.csv"));
        ...
        var arenaDoors = LoadArenaDoors(ResolvePath("NEXUS_ARENA_DOORS", "ArenaDoors.csv"));
        ...
        MusicTracks = LoadMusicTracks(ResolvePath("NEXUS_MUSIC_TRACKS", "MusicTracks.csv"));
        (BgmZones, DefaultBgm) = LoadBgmZones(ResolvePath("NEXUS_MAP_BGM", "MapBgm.csv"));
        _bgmByMap = BuildBgmMap();   // needs Maps + Warps + BgmZones — resolves every map to a track
        Inns = LoadInns(ResolvePath("NEXUS_INNS", "Inns.csv"));
        ForageAreas = LoadForageAreas(ResolvePath("NEXUS_FORAGE", "ForageAreas.csv"));
        PathHalls = LoadPathHalls(ResolvePath("NEXUS_PATHHALLS", "PathHalls.csv"));
        GatewayRegions = LoadGatewayGates(ResolvePath("NEXUS_GATEWAY", "GatewayGates.csv"));
        WorldDests = LoadWorldDests(ResolvePath("NEXUS_WORLDMAP_DESTS", "WorldMapDests.csv"));
        WorldMapTriggers = LoadWorldTriggers(ResolvePath("NEXUS_WORLDMAP_TRIGGERS", "WorldMapTriggers.csv"));
        FallRooms = LoadFallRooms(ResolvePath("NEXUS_FALLROOMS", "FallRooms.csv"));
        BoardLocations = LoadBoardLocations(ResolvePath("NEXUS_BOARD_LOCATIONS", "BoardLocations.csv"));
        ShopCatalogues = LoadShopCatalogues(ResolvePath("NEXUS_SHOP_CATALOGUES", "ShopCatalogues.csv"));
        SpellParams = LoadKeyedRows(ResolvePath("NEXUS_SPELL_PARAMS", "SpellParams.csv"));
        ...
        if (!SpellScript.Load(ResolvePath("NEXUS_SPELL_VERBS", "spell_verbs.lua"))) rejected.Add("spell_verbs.lua");
        ItemParams = LoadKeyedRows(ResolvePath("NEXUS_ITEM_PARAMS", "ItemParams.csv"));   // same "whole row keyed by `key`" shape as SpellParams
        if (!ItemScript.Load(ResolvePath("NEXUS_ITEM_VERBS", "item_verbs.lua"))) rejected.Add("item_verbs.lua");
        if (!NpcScript.Load(ResolvePath("NEXUS_NPC_DIALOG", "npc_dialog.lua"))) rejected.Add("npc_dialog.lua");
        RejectedScripts = rejected;
        // Phase-1 spell-DATA tables (extracted from Content.cs literals; see re/extract_spell_tables.py).
        PetSpells = LoadPets(ResolvePath("NEXUS_PETS", "Pets.csv"));
        WeaponProcs = LoadWeaponProcs(ResolvePath("NEXUS_WEAPON_PROCS", "WeaponProcs.csv"));
        TrapSpells = LoadTrapSpells(ResolvePath("NEXUS_TRAPS", "Traps.csv"));
        (MorphSpells, MorphDispatchSpells) = LoadMorphs(ResolvePath("NEXUS_MORPHS", "Morphs.csv"));
        (RageAmount, EnchantSpells) = LoadSpellMods(ResolvePath("NEXUS_SPELL_MODS", "SpellMods.csv"));
        NpcCompositions = LoadNpcCompositions(ResolvePath("NEXUS_NPC_ABILITIES", "NpcAbilities.csv"));
        PathGrowth = LoadPathGrowth(ResolvePath("NEXUS_PATH_GROWTH", "PathGrowth.csv"));
        (DoorSwaps, DoorDeltas, DoorDefaultOpen) = LoadDoorObjects(ResolvePath("NEXUS_DOOR_OBJECTS", "DoorObjects.csv"));
        Tuning = LoadTuning(ResolvePath("NEXUS_SERVER_TUNING", "ServerTuning.csv"));
        Doors.SetConfig(LoadDoors(ResolvePath("NEXUS_DOORS", "Doors.csv")));
        (_mapCells, var mapCellCount) = LoadMapCells(ResolvePath("NEXUS_MAP_CELLS", "MapCells.csv"));
