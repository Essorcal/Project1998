...
public static class Doors
{
    ...
    /// programmatically as well as with a key, and for doors whose open/closed pairing simply CAN'T be
    /// expressed by id (see <see cref="Toggle"/>). The run starts at this tile + <c>StartDx</c> and runs
    /// right; both arrays must be the same length. <c>DefaultClosed</c> stamps <c>ClosedObjs</c> on at load so
    /// the door starts shut however the shipped map draws it — pair it with <c>Locked</c> for "shut and locked
    /// until the event opens it".</summary>
    ...
    public static IEnumerable<(ushort x, ushort y, ushort[] objs)> DefaultClosedRuns(ushort map) =>
        Config.Where(kv => kv.Key.map == map && kv.Value.DefaultClosed && kv.Value.ClosedObjs is { Length: > 0 })
              .Select(kv => ((ushort)(kv.Key.x + kv.Value.StartDx), kv.Key.y, kv.Value.ClosedObjs!));

    /// <summary>The per-tile open/close swap for a door the player is facing: where its run starts and which
    /// ids to stamp, given what is on the map there now. Null when this tile has no explicit run configured,
    /// which sends <c>Session.HandleOpen</c> to RTK's shared id-swap table (<c>Content.DoorToggleFor</c>) as
    /// before.
    ///
    /// <para>Explicit per-tile runs exist because the shared table is keyed by OBJECT ID alone, and some doors
    /// can't be described that way. Arctic Village's Laptev Crafter is the case in point: RTK's
    /// <c>open.lua</c> has no rule for ids 356-359 at all, and the open leaves it borrows (366/367) are
    /// already the open form of 346/347 — so an id rule would make one door's close swap another door's
    /// graphic. Keyed by tile, both doors coexist.</para>
    ///
    /// <paramref name="objAt"/> reads the CURRENT object id at a cell, so this can tell open from closed by
    /// looking. A run that matches neither (a GM edit, a half-applied patch) is treated as closed and opens,
    /// which is the recoverable direction — the alternative leaves a door nothing can shift.</summary>
    public static (int StartX, ushort[] Objs)? Toggle(ushort map, ushort x, ushort y, Func<int, ushort> objAt)
    {
        if (For(map, x, y) is not { ClosedObjs: { Length: > 0 } closed, OpenObjs: { Length: > 0 } open } cfg)
            return null;
        int sx = x + cfg.StartDx;
        bool isOpen = true;
        for (int i = 0; i < open.Length; i++) if (objAt(sx + i) != open[i]) { isOpen = false; break; }
        return (sx, isOpen ? closed : open);
    }
